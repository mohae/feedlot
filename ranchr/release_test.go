...
func TestUbuntuSetISOInfo(t *testing.T) {
	...
	if err != nil {
		...
		if u.isoURL != "http://releases.ubuntu.com/14.04/ubuntu-14.04-server-amd64.iso" {
			t.Errorf("Expected \"http://releases.ubuntu.com/14.04/ubuntu-14.04-server-amd64.iso\", got %q", u.isoURL)
		}
		...
		if u.Name != "ubuntu-14.04-server-amd64.iso" {
			t.Errorf("Expected \"ubuntu-14.04-server-amd64.iso\", got %q", u.Name)
		}
	}
	...
	err = u.SetISOInfo()
	if err == nil {
		t.Error("Expected error: \"Unable to find ISO information while looking for the release string on the Ubuntu checksums page.\", none received")
	} else {
		if err.Error() != "unable to find ISO information while looking for the release string on the Ubuntu checksums page" {
			t.Errorf("Expected \"unable to find ISO information while looking for the release string on the Ubuntu checksums page\", got %q", err.Error())
		}
	}
}
...
func TestUbuntuSetChecksum(t *testing.T) {
	...
	err = u.setChecksum()
	if err == nil {
		t.Error("Expected error: \"Get http://releasea.ubuntu.com/14.04/ABCSUMS: dial tcp: lookup releasea.ubuntu.com: no such host\", none received")
	} else {
		if err.Error() != "Get http://releasea.ubuntu.com/14.04/ABCSUMS: dial tcp: lookup releasea.ubuntu.com: no such host" {
			t.Errorf("Expected \"Get http://releasea.ubuntu.com/14.04/ABCSUMS: dial tcp: lookup releasea.ubuntu.com: no such host\", got %q", err.Error())
		}
	}
	...
	err = u.setChecksum()
	if err == nil {
		t.Error("Expected error: \"Unable to retrieve checksum while looking for ubuntu-14.04-random-amd64.iso on the Ubuntu checksums page.\", got nil")
	} else {
		if err.Error() != "Get http://releasea.ubuntu.com/14.04/ABCSUMS: dial tcp: lookup releasea.ubuntu.com: no such host" {
			t.Errorf("Expected error: \"Get http://releasea.ubuntu.com/14.04/ABCSUMS: dial tcp: lookup releasea.ubuntu.com: no such host\", got %q", err.Error())
		}
	}
}
...
func TestCentOSISORedirectURL(t *testing.T) {
	...
	expected := "http://isoredirect.centos.org/centos/" + c.Release + "/isos/" + c.Arch + "/"
	if redirectURL != expected {
		t.Errorf("Expected %q, got %q", expected, redirectURL)
	}
}
...
func TestSetISOInfo(t *testing.T) {
	...
	if err != nil {
		...
		if c.Name == "" {
			t.Error("Expected Name to not be empty, but it was")
		}
		if !strings.HasPrefix(c.isoURL, "http://") {
			t.Errorf("Expected %q to start with \"http://\"", c.isoURL)
		}
		if !strings.HasSuffix(c.isoURL, ".iso") {
			t.Errorf("Expected %q to end with \".iso\"", c.isoURL)
		}
	}
	...
	if err == nil {
		...
		if err.Error() != "arch for  was empty, unable to continue" {
			t.Errorf("Expected \"arch for  was empty, unable to continue\", got %q", err.Error())
		}
	}
	...
	if err == nil {
		...
		if err.Error() != "release for  was empty, unable to continue" {
			t.Errorf("Expected \"release for  was empty, unable to continue\", got %q", err.Error())
		}
	}
	...
	if err == nil {
		...
		if err.Error() != "Get http://invalid/6.6/isos/x86_64/sha256sum.txt: dial tcp: lookup invalid: no such host" {
			t.Errorf("Expected \"Get http://invalid/6.6/isos/x86_64/sha256sum.txt: dial tcp: lookup invalid: no such host\", got %q", err.Error())
		}
	}
	...
	if err == nil {
		...
		if err.Error() != "Checksum Type not set" {
			t.Errorf("Expected \"Checksum Type not set\", got %q", err.Error())
		}
	}
}
