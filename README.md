...
One the Packer template(s) are generated by Rancher, you will need to use [Packer](https://packer.io).
...
Build a Packer template from a named build:
    ...
    rancher build 1494-amd64

Build a Packer template for Ubuntu using the distro defaults and the 32bit image and from more than one named build:  
	
    rancher build -distro=ubuntu -arch=i386 1404-amd64 jessie-amd64 centos6-x86_64
...
Some Packer templates may have multiple provisioners. Some provisioners require work to be done by another provisioner first, like Puppet. Unfortunately, due to the way Go writes out JSON, this ordering will not be preserved, all JSON is written in alphabetical order. For situations where the Provisioner order is important, the generated Packer template will need to be modified.
